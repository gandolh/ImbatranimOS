...
/**
 * windowId → the file that window is showing. The open payload is latched here
 * (a zustand setter, not React setState) so consuming the one-shot intent
 * happens outside React's render/effect state churn — the same pattern Notepad
 * uses, and StrictMode-safe.
 */
