...
- [status](wiki/status.md) — Dated snapshot — web-OS era; briefs 08–14 + 16–33 DONE (incl. the 2026-07-17 post-v1 backlog run, the review-pass cleanup wave 23–30, and the daily-driver perf trio 31–33: list virtualization, xlsx off-thread worker, and app-shell lazy-load which cut the eager bundle −69.6%); brief 15's human-gated remainder is all that stands before v1.0. A full-auto daily-driver app backlog is mid-run (briefs 34–46: notification-center → light apps → heavy/backend apps → platform surfaces). Held human-gated: SEC-9 CSP + SEC-10 kiosk sandbox (browser/ISO-gated), brief 15 v1-release remainder.
