...
import { PtySession, type SocketLike } from './pty-session';
...
export class PtyGateway implements OnApplicationBootstrap, OnModuleDestroy {
  ...
  onApplicationBootstrap(): void {
    ...
    this.upgradeHandler = (req, socket, head) => {
      ...
      const record = authorizeUpgrade(req, this.sessions);
      if (!record) {
        this.logger.warn('Rejected unauthenticated terminal upgrade');
...
function parseGeometry(url: string | undefined): { cols: number; rows: number } {
  ...
  if (q) {
    ...
    if (Number.isFinite(c) && c > 0) cols = Math.floor(c);
    if (Number.isFinite(r) && r > 0) rows = Math.floor(r);
  }
  return { cols, rows };
}
